...
function eventLabels(shot) {
  ...
  switch (visual.kind) {
    ...
    case 'evidence': return ['primary source', ...(visual.callouts ?? [])];
    ...
    case 'swimlane': return (visual.events ?? []).map(event => typeof event === 'string' ? event : event.label);
...
const shots = timeline.shots.map(timing => {
  ...
  if (explicit && explicit.length !== labels.length) {
  ...
  const events = labels.map((label, index) => {
    const proportional = labels.length === 1 ? 0 : Math.round(index * (shotCues.length - 1) / (labels.length - 1));
    const cueIndex = explicit?.[index] ?? proportional;
    ...
    return {
      ...
      strategy: explicit ? 'explicit' : 'proportional',
    };
  });
