...
check('case-callback', story.shots.at(-1)?.hostNarrative?.role === 'resolve' && story.narrativeCase?.callback, story.narrativeCase?.callback ?? 'missing');
...
const report = {schemaVersion: '1.0', generatedAt: new Date().toISOString(), result: failures.length ? 'fail' : 'pass', metrics: {hostRatio, demoRatio, demoSeconds, duration: timeline.duration}, checks};
